...
echo "Please log in to access these rights! <script> window.location='http://www.fossclubgoa.com/chowgulefossclub/feedbackproject/feedback_login.php'; </script>";
  ...
  include 'con.php';
  $fid = $_GET['id'];
  $result = $con->query("select * from user where u_id in (select distinct(user_id) from generalAnswers where question_id in (select q_id from questions where form_id=$fid))");
      ...
      <?php
        while($row = $result->fetch_assoc()){
          echo "  <div class=\"row forms\">
              <div class=\"col-md-8\"><a href=\"feedback_view_data.php?uid=".$row['u_id']."&fid=".$fid."\">".$row['fname']." ".$row['lname'] ."</a></div>
            </div>";
        }
      ?>
